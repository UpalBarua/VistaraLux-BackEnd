...
export const doSubscribe = async (req, res, next) => {
    try {
        ...
        if (!name || !email)
            return next(new ErrorHandler("Please enter your name and email to subscribe VistaraLux", 400));
        const isSubscribed = await SubscriberModel.findOne({ email });
        if (isSubscribed)
            return next(new ErrorHandler("No need to, you've already subscription", 400));
...
export const allSubscriber = async (req, res, next) => {
    try {
        const subscribers = await SubscriberModel.find({});
        if (subscribers.length < 1)
            return next(new ErrorHandler("Oops! There's no subscriber yet.", 404));
        res.status(200).json({
            success: true,
