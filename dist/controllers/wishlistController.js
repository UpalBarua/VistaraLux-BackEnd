...
export const addToWishlist = async (req, res, next) => {
    ...
    try {
        ...
        if (existingWishlist) {
            const productExists = existingWishlist.products.some((item) => item.productId.toString() === products);
            if (productExists) {
                return next(new ErrorHandler("This product is already in your wishlist", 409));
            }
        }
        // Logic for adding product to the user's wishlist
        const wishlist = await WishlistModel.findOneAndUpdate({ user: user }, // user is the string reference to the user
...
export const wholeWishlistForAdmin = async (req, res, next) => {
    try {
...
export const removeFromWishlist = async (req, res, next) => {
    ...
    try {
        ...
        const updatedWishlist = await WishlistModel.findOneAndUpdate({ user: userId }, // Match the user's wishlist
