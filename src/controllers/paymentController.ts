...
export const createCoupon = async (
    ...
    try {
        ...
        if (isCouponAlreadyExist) return next(new ErrorHandler("This coupon is already exist", 409))
...
export const getAllCoupons = async (
    ...
    try {
        ...
        if (coupons.length < 1) return next(new ErrorHandler("Oops! There's no coupon right now", 404))
        res.status(200).json({
            success: true,
